...
export const GetAllBuyerReservations = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      ...
      let ReservationsUrl = `${import.meta.env.VITE_API_BASE_URLS}/api/v1/reservations`;
...
export const GetAllHostReservations = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      ...
      let ReservationsUrl = `${import.meta.env.VITE_API_BASE_URLS}/api/v1/reservations/host`;
      const { data } = await axios.get(ReservationsUrl, config);
      return data.reservations;
    } catch (error) {
      return thunkAPI.rejectWithValue(
...
export const GetSingleBuyerReservations = createAsyncThunk(
  ...
  async (name, thunkAPI) => {
    ...
    try {
      let ReservationsUrl = `${import.meta.env.VITE_API_BASE_URLS}/api/v1/reservations/buyer/${name}`;
      const { data } = await axios.get(ReservationsUrl, config);
      return data.reservations;
    } catch (error) {
      return thunkAPI.rejectWithValue(
  ...
  async (reservationdata, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${import.meta.env.VITE_API_BASE_URLS}/api/v1/reservations/${state.gigs.GigsDetails._id}`,
        reservationdata,
        config
      );
  ...
  async (GigsData, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${import.meta.env.VITE_API_BASE_URLS}/api/v1/reservations/${_id}`,
        GigsData,
        config
      );
  ...
  async (Reservationsid, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.delete(
        `${import.meta.env.VITE_API_BASE_URLS}/api/v1/reservations/${Reservationsid}`,
        config
      );
