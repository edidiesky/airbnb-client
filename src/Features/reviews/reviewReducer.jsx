...
export const CreateSingleReview = createAsyncThunk(
  ...
  async (ReviewsData, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.post(`${import.meta.env.VITE_API_BASE_URLS}/api/v1/review`, ReviewsData, config);
  ...
  async ({ReviewsData}, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${import.meta.env.VITE_API_BASE_URLS}/api/v1/review/${_id}`,
        ReviewsData,
        config
      );
      // console.log(ReviewsData)
  ...
  async (Reviewsid, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.delete(
        `${import.meta.env.VITE_API_BASE_URLS}/api/v1/review/${Reviewsid}`,
        config
      );
      return Reviewsid;
    } catch (error) {
  ...
  async ({ Reviewdata, id }, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${import.meta.env.VITE_API_BASE_URLS}/api/v1/review/review/${id}`,
        Reviewdata,
        config
      );
      return data.message;
...
export const getTopRatedReviews = createAsyncThunk(
  ...
  async (name, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.get(`${import.meta.env.VITE_API_BASE_URLS}/api/v1/review/rated`, config);
...
export const getReviewsStats = createAsyncThunk(
  ...
  async (name, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.get(`${import.meta.env.VITE_API_BASE_URLS}/api/v1/review/stats`, config);
      return data.stats;
    } catch (error) {
      return thunkAPI.rejectWithValue(
        error.response && error.response.data.message
