...
export const getAllCustomer = createAsyncThunk(
  ...
  async (name, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.get(`${import.meta.env.VITE_API_BASE_URLS}/api/v1/user?page=${userpage}`, config);
      return data;
    } catch (error) {
      return thunkAPI.rejectWithValue(
        error.response && error.response.data.message
  ...
  async (name, thunkAPI) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${import.meta.env.VITE_API_BASE_URLS}/api/v1/user/profile/${name}`,
        config
      );
      return data;
    } catch (error) {
